...
def calculate_boxsize_script(pdb_string):
    with [tempfile.NamedTemporaryFile(mode="w+t"),
          tempfile.NamedTemporaryFile(mode="w+t")] \
         as [pdb_tmp, leap_tmp]:

        # prepare pdb file
        pdb_tmp.write(pdb_string)
        pdb_tmp.flush()
        pdb_filepath = pdb_tmp.name

        # prepare tleap input file
        leap_tmp.write(tmp_leap.format(pdbfile=pdb_filepath,
                                       tmp_prefix=".tmp"))
        leap_tmp.flush()

        # run tleap
        result_tmp = tempfile.mkstemp(prefix=".tmp")
        out = gop(f"tleap -f {leap_tmp.name} | tee {result_tmp}")
        center_str = gop(f"cat {result_tmp} | grep The\ center | cut -d: -f2 | sed s/,//g")
        center = [float(s) for s in center_str.split()]
        gop(f"rm {result_tmp}")

    # get box size
    box_size_str = gop("tail -n 1 .tmp.rst7 | cut -c -36")
    try:
...
def run_parmchk_script(mol2, at):
    at_id = {"gaff": 1, "gaff2": 2}[at]    
    with tempfile.NamedTemporaryFile(mode="w+t") as mol2_tmp:
        # prepare mol2 file
        mol2_tmp.write(mol2)
        mol2_tmp.flush()

        # run parmchk2
        frcmod_tmp = tempfile.mkstemp(prefix=".tmp", suffix=".frcmod")
        print(gop(f"parmchk2 -i {mol2_tmp.name} -f mol2 -o {frcmod_tmp} -s {at_id}"))
        frcmod = gop(f"cat {frcmod_tmp}")
    return frcmod
...
def gen_packmol_input_script(prot_pdb, cosolv_pdbs, box_pdb, box_size, molar, seed=-1):
    protein_pdb_filepath = tempfile.mkstemp(prefix=".tmp", suffix=".pdb")
    with open(protein_pdb_filepath, "w") as fout:
        fout.write(prot_pdb)

    cosolv_pdb_filepathes = [tempfile.mkstemp(prefix=".tmp", suffix=".pdb") for _ in cosolv_pdbs]
    for cosolv_pdb_filepath, cosolv_pdb in zip(cosolv_pdb_filepathes, cosolv_pdbs):
        with open(cosolv_pdb_filepath, "w") as fout:
            fout.write(cosolv_pdbs)

    num = int(constants.N_A * molar * (box_size**3) * (10**-27))

    output_filepath = tempfile.mkstemp(prefix=".tmp", suffix=".pdb")
    ret = TEMPLATE_PACKMOL_HEADER.format(output=output_filepath,
                                         prot=protein_pdb_filepath, seed=seed)
    ret += "\n"
    for cosolv_pdb_filepath in cosolv_pdb_filepathes:
        ret += TEMPLATE_PACKMOL_STRUCT.format(cosolv=cosolv_pdb_filepath,
                                              num=num, size=box_size/2)
        ret += "\n"
    return ret, output_filepath
...
def run_packmol_script(packmol_path, inp):
    with tempfile.NamedTemporaryFile(mode="w+t") as packmol_input_file:
        # prepare packmol input file
        packmol_input_file.write(inp)
        packmol_input_file.flush()

        # run packmol
        print(gop(f"{packmol_path} < {packmol_input_file.name}"))
    return
...
# TODO:
# ・templateは常に同一のものを使うのでは？
# ・表に出す関数についてはpathのやり取りを避ける
#   ・parmchkを走らせるのはこの関数の内部ということになる
def gen_tleap_input_script(template_file, inputfile, cids, cosolv_paths, frcmods, box_path, size, oprefix, ssbonds, at):
    data = {
...
def run_tleap_script(cmd, inp_str):
    tmp_inp_path = tempfile.mkstemp(prefix=".tmp", suffix=".in")
    with open(tmp_inp_path, "w") as fout:
        fout.write(inp_str)

    output = gop("%s -f %s" % (cmd, tmp_inp_path))
    print(output)

    # TODO: 出力されたファイルの文字列をreturnしたい。
