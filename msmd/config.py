...
class _InputConfig:
    ...
    def __parse_protein(protein_map: Dict[str, Any]) -> Protein:
        __pdb: str = protein_map["pdb"]
        __ssbond: List[Tuple[int, int]] = protein_map["ssbond"]
        return Protein(__pdb, __ssbond)

    @staticmethod
    def __parse_probe(probe_map: Dict[str, str]) -> Probe:
        __cid = probe_map["cid"]
        __mol2 = probe_map["mol2"]
        __pdb = probe_map["pdb"]
        return Probe(__cid, __mol2, __pdb)
...
class _SimulationStepFactory:
    ...
    def create(self, step_config: Dict[str, Any]) -> SimulationInterface:
