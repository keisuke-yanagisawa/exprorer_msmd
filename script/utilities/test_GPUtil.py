...
def _make_gpu(gpu_id, load=0.0, memory_util=0.0):
    """Create a mock GPU object for testing"""
    gpu = MagicMock()
    gpu.id = gpu_id
    gpu.load = load
    gpu.memoryUtil = memory_util
    return gpu
...
def test_get_gpuids_no_gpu(mock_gpu_env):
    ...
    """
    mock_gpu_env['get_gpus'].return_value = []
...
def test_get_gpuids_single_gpu(mock_gpu_env):
    ...
    Expected: Returns [0]
    """
    mock_gpu_env['get_gpus'].return_value = [_make_gpu(0)]
    mock_gpu_env['getenv'].return_value = None
...
def test_get_gpuids_multiple_gpus(mock_gpu_env):
    ...
    """
    mock_gpu_env['get_gpus'].return_value = [_make_gpu(0), _make_gpu(1), _make_gpu(2)]
    mock_gpu_env['getenv'].return_value = None

    gpuids = get_gpuids()

    assert set(gpuids) == {0, 1, 2}
...
def test_get_gpuids_with_cuda_visible_devices(mock_gpu_env):
    ...
    """
    mock_gpu_env['get_gpus'].return_value = [_make_gpu(0), _make_gpu(1), _make_gpu(2), _make_gpu(3)]
...
def test_get_gpuids_nan_load_mig_gpu(mock_gpu_env):
    """
    Test for MIG-enabled GPU where load is nan
    Expected: GPU with nan load is treated as available
    """
    mock_gpu_env['get_gpus'].return_value = [_make_gpu(0, load=float('nan'), memory_util=0.07)]
    mock_gpu_env['getenv'].return_value = None
