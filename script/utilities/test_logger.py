...
from .logger import __DEFAULT_LOG_LEVEL__, Logger
...
class TestLoggerInitialization:
    ...
    def test_default_initialization(self, logger):
        ...
        assert any(
            isinstance(h, StreamHandler) and not isinstance(h, handlers.RotatingFileHandler)
    ...
    def test_file_initialization(self, file_logger, log_file):
        ...
        # Check for standard output handler
        assert any(
            isinstance(h, StreamHandler) and not isinstance(h, handlers.RotatingFileHandler) for h in handlers_list
        )
...
class TestLoggerOutput:
    ...
    @pytest.mark.parametrize(
        ...
        [("debug", "DEBUG"), ("info", "INFO"), ("warn", "WARNING"), ("error", "ERROR"), ("critical", "CRITICAL")],
    )
    ...
    def test_log_format(self, file_logger, log_file):
        ...
        with open(log_file, "r") as f:
            ...
            assert match is not None, f"Log format does not match.\nExpected: {pattern}\nActual: {content}"
...
class TestLoggerHandlers:
    ...
    def test_handler_synchronization(self, logger):
        ...
        for level in test_levels:
            ...
            assert logger.handler.level == logger.logger.level, f"Handler is not synchronized at level {level}"

    def test_handler_formatter(self, logger):
        """Handler formatter verification test"""
        expected_format = "[%(asctime)s] [%(levelname)s] %(message)s"
        assert (
            logger.handler.formatter._fmt == expected_format
