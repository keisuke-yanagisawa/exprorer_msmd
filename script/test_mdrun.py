...
def general_settings():
    ...
    return {"temperature": 300, "pressure": 1.0, "dt": 0.002, "pbc": "xyz"}
...
class TestGenMdp:
    ...
    def test_gen_mdp_protocols(self, tmp_path, mock_jinja, protocol):
        ...
        mock_env.return_value.get_template.assert_called_once_with(f"{protocol['type']}.mdp")

        # Verify protocol parameters are passed correctly (render receives a copy, not the original)
        render_args = mock_template.render.call_args[0][0]
        for key, value in protocol.items():
            assert render_args[key] == value

        # For heating type, verify additional parameters in the rendered dict
        if protocol["type"] == "heating":
            ...
            assert render_args["target_temp"] == expected_target_temp
            assert render_args["initial_temp"] == expected_initial_temp
            assert render_args["duration"] == pytest.approx(expected_duration)

            # Verify that the original dict was NOT mutated
            if "duration" not in protocol:
                assert "duration" not in protocol
    ...
    def test_gen_mdp_invalid_types(self, tmp_path, invalid_type):
        ...
        protocol = {"type": invalid_type, "name": "test"}
...
class TestPrepareSequence:
    def test_prepare_sequence_protocol(self, general_settings):
        ...
        sequence = [
            {"type": "minimization", "name": "min", "define": "-DPOSRES1000", "nsteps": 200},
            {
                "type": "production",
                "name": "pr",
                "define": "",
...
class TestPrepareMdFiles:
    def test_prepare_md_files_protocol(self, tmp_path, mock_jinja):
        ...
        sequence = [
            {"type": "minimization", "name": "min", "define": "-DPOSRES1000"},
            {"type": "production", "name": "pr", "define": ""},
        ]
...
class TestDetectMdrunThreadFlag:
    ...
    def test_thread_mpi_build(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/gmx"), patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(stdout="GROMACS version: 2024\nMPI library:      thread_mpi\n", stderr="")
            assert detect_mdrun_thread_flag(Path("gmx")) == "-nt"

    def test_real_mpi_build(self):
        """real MPI build should return -ntomp"""
        with patch("shutil.which", return_value="/usr/bin/gmx_mpi"), patch("subprocess.run") as mock_run:
    ...
    def test_version_timeout(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/gmx"), patch(
            "subprocess.run", side_effect=subprocess.TimeoutExpired("gmx", 30)
        ):
